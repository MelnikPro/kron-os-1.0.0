...
echo "=== Фикс output.c ==="
cat > $BASE/compositor/output.c << 'EOF'
#include "output.h"
#include "server.h"
#include <stdlib.h>
#include <time.h>
...
echo "=== Фикс server.c setenv ==="
sed -i 's/setenv("WAYLAND_DISPLAY", socket, true)/setenv("WAYLAND_DISPLAY", socket, 1)/' $BASE/compositor/server.c
grep -q '#include <stdlib.h>' $BASE/compositor/server.c || sed -i '/#include <unistd.h>/a #include <stdlib.h>' $BASE/compositor/server.c

echo "=== Фикс toplevel.c double->int ==="
sed -i 's/double lx, ly;/int lx, ly;/g' $BASE/compositor/toplevel.c

echo "=== Сборка ==="
cd $BASE
ninja -C build && echo "=== KRON СОБРАН УСПЕШНО ===" || echo "=== ОШИБКИ ВЫШЕ ==="
